---
title: 333.2 - Mandatory Access Control
---
...
### [runcon](https://www.gnu.org/software/coreutils/runcon)
Switch SELinux context for a command, or print the current context if no arguments are specified.
...
user@host:~$ runcon -u system_u -l s0:c511 $(command -v runcon)
system_u:unconfined_r:unconfined_t:s0:c511
```
Legend:
* user: unconfined_u
* role: unconfined_r
* type: unconfined_t
* level:
  * sensitivity: s0-s0 ("range" of s0 through s0)
  * category: c0.c1023 (contiguous series of c0 through c1023)
